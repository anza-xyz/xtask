...
mod tests {
    ...
    fn test_publish_excluded_packages() {
        ...
        let names: Vec<&str> = data
            ...
            .map(|p| p.name.as_str())
            .collect();
        assert!(names.contains(&"publishable"), "publishable should be included");
        assert!(!names.contains(&"excluded"), "excluded (publish=[]) should be filtered out");
    ...
    fn test_publish_order_exact_levels() {
        ...
        assert_eq!(name_to_level["d"], 3, "d depends on a and c, so it must be last");
    }
    ...
    fn test_publish_order_tree_with_dependencies() {
        ...
        assert!(result.is_ok(), "Tree output with dependencies should succeed");
    }
